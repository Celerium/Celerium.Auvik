...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('APIKeys') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "[ Test-AuvikAPICredential ] with a bad API key should fail to authenticate" {
            Add-AuvikBaseUri
            Add-AuvikAPIKey -Username 'Celerium@Celerium.org' -ApiKey "AuvikKey"

            $Value = Test-AuvikAPICredential -ErrorAction SilentlyContinue -WarningAction SilentlyContinue #3>$null
